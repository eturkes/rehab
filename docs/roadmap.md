...
* **Known-good** — ruff clean, pytest green (32). Stack refreshed (F27, done): scikit-learn 1.9.0, shap 0.52.0, lightgbm 4.6.0, numpy 2.4.6, pytest 9.1.0; pip-audit clean. The prior `sklearn 1.9→1.8` bundle skew is resolved — all served bundles re-pickled at 1.9.0; a rescan of every `models/**/*.joblib` (66) loads clean after removing one gitignored, unreferenced orphan (`conversion/lgbm_binary.joblib`, stale 1.8.0). pytest 9.1.0 closes CVE-2025-71176. F27 also corrected stale `landmark_metrics.json` + `temporal_metrics.json` (pre-G9, 6 → all 11 outcomes). Review confirmed the retrain: production held-out/conformal/SHAP byte-identical (fixed-seed `GroupShuffleSplit`); G-series served metrics legitimately recomputed (OOF via `GroupKFold`, no collapse — headline AUC/Brier moved ≤~0.05).
