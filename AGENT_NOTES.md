...
* `.claude/commands/session-prompt.md` — the `/session-prompt` slash command:
  reusable bootstrap for a new session.  Invoke bare (`/session-prompt`) to
  pull the next backlog item, or `/session-prompt <TASK>` to steer a one-off
  task that overrides the roadmap for that run.  Update if the workflow changes.
