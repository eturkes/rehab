...
* **Known-good** — ruff clean, pytest green (32). Stack refreshed (F27, done): scikit-learn 1.9.0, shap 0.52.0, lightgbm 4.6.0, numpy 2.4.6, pytest 9.1.0; pip-audit clean. The prior `sklearn 1.9→1.8` bundle skew is resolved (all artifacts re-pickled at 1.9.0, no unpickle warning). F27 also corrected stale `landmark_metrics.json` + `temporal_metrics.json` (pre-G9, 6 outcomes → all 11).
