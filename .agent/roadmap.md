...
* **Dashboard** (Plotly Dash, JA default / EN toggle) — findings-first landing with four source-bound findings on unfiltered data, glance-first: one-line deck + scope stats, then a 4-tile glance band (one-line claim, anchor→block; tile order == block order, own wording, test-gated) putting all four findings in one viewport; each block visible = numbered kicker + claim as headline + scope line + self-annotated figure (muted-middle emphasis, +gain annotation, shaded uncertainty band w/ −shrink label) — no display numeral anywhere in the copy layer, because every finding is a relation and each figure already labels its own values ([project reference §4](../docs/project-reference.md#4-dashboard-conventions)); reviewed takeaway/reading/basis prose complete behind the per-finding disclosure (gate caps claim <170 / scope <125 chars, floors basis >200); shared ground-rules note collapsed under the lead; collapsed admission-severity gradient; separate filtered cohort explorer. Typography = self-hosted IBM Plex subsets (sans + mono). Story tab order overview→insights→patient→simulator→methods. Patient tab: prognosis full-width, ISNCSCI table behind a disclosure. Simulator: F25 completeness/OOD badge + cohort-baseline note on zero-input (test-gated); "fill defaults" labelled illustrative. What-if reference overlay.
...
* **Known-good** — ruff clean, pytest green (56), pip-audit clean. Stack: scikit-learn 1.9.0, shap 0.52.0, lightgbm 4.6.0, numpy 2.4.6, pytest 9.1.0 (CVE-2025-71176 floor). All 66 `models/**/*.joblib` are pickled at sklearn 1.9.0 and load clean; `landmark_metrics.json` + `temporal_metrics.json` cover all 11 outcomes. Which metrics survive a retrain or a sklearn bump byte-identical: [project reference §0b](../docs/project-reference.md#0b-lessons-and-pitfalls).
