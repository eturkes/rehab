...
- **`reverted_of_peaked` = fell below its OWN peak, not back to admission** (`(best > last)[peaked].mean()`). Adm A: 34.1% = 28/82 fell below peak, but only 16/82 (19.5%) were back at A — 12 landed C→B and stayed above admission. Return-to-admission-or-below = `(n_peaked − n_held)/n_peaked`. Misreading the field as return-to-baseline shipped a wrong headline once.
- Three reversion denominators, all live: `reverted_of_peaked` (per grade, conditional on crossing), `overall.fell_back` 7.7% = 53/686 = any drop from peak incl. non-crossers (40 crossings + the 13 non-crossers that end below admission; CLI summary only — the figure labels counts), `rate_ended_below` 1.9% = 13/686. Never mix them in one claim.
- `flow.dips` + per-triple `drops` = drop **timing**. 108 episodes were ever recorded below their own peak = 53 `fell` (still below at the last assessment — the same 53 as `fell_back`) + 55 `recovered` (peak == last, so they are invisible in every other `flow` field; a timing axis built off `fell` alone halves the event count). Drop slot = first window slot below the peak **after** the peak's first occurrence, because the climb towards a peak is not a loss of it. Figure buckets ≤4w 51 · 6w–3m 32 · 4–6m 25; median first record day 60 (fell) / 28 (recovered); per-slot hazard 7.7% (72h) → 4.5% (6m) ⇒ drops sit across the whole window, not at one moment.
- The slot says **when the drop was recorded, not when it occurred** — interval-censored upper bound (the loss lies between the last at-peak assessment and the one that caught it) — and follow-up length bounds it (a record ending at 2m cannot show a later drop). Both caveats ride in `improve_basis`; without them the markers read as event times.
- Reversion is wildly asymmetric but the per-grade contrast is thin at one end: adm A 28/82 crossings do not hold vs adm D 1/43 → **pool the other grades (12/251) for any contrast**, never headline the D arm. **Never gloss non-holding as lost recovery** — the A→B boundary turns on one sacral finding, the measurement most exposed to inter-rater disagreement, so instability and exam disagreement are not separable here. `basis` carries both the pooled counts and that caveat.
...
- Figure-only iteration skips the server: `BROWSER_PATH="$(chromiumfish path)" uv run python …` + `fig.write_image(path, width=W, height=H, scale=1.6)`, then `Read` the PNG. Kaleido drives that same chromiumfish binary; without `BROWSER_PATH` it tries to fetch its own Chrome. Shoot 980 **and** 680 px — the live chart column runs ~1000–1300 px at ordinary viewports, and 680 is the stress width where Sankey node labels (which run rightwards from the node) collide first. Anything CSS-bound — wrap, resolved face, `lang` — still needs the Playwright harness.
- Plotly Sankey `arrangement="snap"` reads node `x`/`y` as **hints** and repacks with `pad` offsets, so a y derived analytically from link stacking lands off by the padding and cannot be checked in closed form. Working rule: give a node the mean of the ribbon midpoints it carries, then prove it by render. Node labels run rightwards from the node — a label belonging over a narrow node goes in a paper-coordinate annotation instead.
