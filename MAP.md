...
### compute.py (851 lines)
Pure computation helpers for model inference, conformal PI, and SHAP.
- L37 `resolve_group_q(q_by_group, marginal, X)` — Resolve Mondrian q for a single-row input.
- L60 `resolve_conformal_q(fspec, X)`
- L68 `resolve_aps_q(fspec, X)`
- L77 `predict_trajectory(X)` — Predict SCIM-total at each trajectory timepoint for a single-row input.
- L114 `aps_prediction_set(proba_row, q_hat)`
- L127 `inv_transform_scalar(x, transform)`
- L133 `clip_scalar(x, lo, hi)`
- L141 `format_value(col, value)`
- L150 `compute_ref_predictions(X)` — Compute predictions for all outcomes on a single-row X.
- L189 `collect_sim_inputs(num_vals, num_ids, cat_vals, cat_ids)`
- L208 `shap_for_row_regression(X, model)`
- L221 `shap_for_row_class(X, clf, class_idx, n_classes)`
- L250 `episode_row_for_model(key_record)` — Build a one-row model input from an episode's admission features.
- L266 `episode_has_admission(key_record)`
- L271 `get_observed_for_outcome(key_record, spec)`
- L284 `_landmark_input(X_base, observed, feature_cols)` — Build a one-row model input over ``feature_cols`` from base features + observed …
- L307 `_predict_landmark_head(head, X, task, transform, cmin, cmax)`
- L334 `predict_landmark(outcome_key, landmark, X_base, observed)` — Paired admission-only baseline vs landmark prediction for one outcome at landmar…
- L361 `landmark_voi(outcome_key, landmark, X_base, observed)` — Per-measure value-of-information for one patient at landmark ``L`` (G2).
- L431 `_episode_timepoint_oidx(key_record)` — Episode rows restricted to the landmark measures, plus the timepoint order-index…
- L441 `landmark_observed_for_episode(key_record, landmark)` — Real LOCF observed block for one episode: last non-null value of each landmark m…
- L460 `episode_landmark_eligibility(key_record)` — Per-landmark still-admitted eligibility: True when the episode has a tracked obs…
- L477 `_phenotype_episode_obs(key_record)` — This episode's observed ``(timepoint, value)`` pairs within the phenotyping wind…
- L493 `phenotype_cutoff_options(key_record, min_cells)` — Window timepoints (chronological) eligible as observation-cutoffs for phenotype
- L517 `predict_phenotype_membership(key_record, cutoff)` — Soft phenotype membership for one episode using only observations on/before ``cu…
- L587 `_apply_platt(calibrator, prob)` — Platt (sigmoid) recalibration over the LightGBM logit — mirrors models.conversio…
- L594 `_conversion_input(X)` — One-row model input over the conversion bundle's feature universe, dtyped like t…
- L609 `mag_short_label(code, mag_cap)` — Compact, language-neutral label for an ordinal improvement-magnitude class.
- L618 `predict_conversion(X)` — Admission->discharge AIS conversion for one admission row (see header).
- L686 `_multistate_input(X)` — One-row improve-head input over the multi-state bundle's feature universe, dtype…
- L701 `multistate_observed_grades(key_record)` — This episode's observed AIS grade (ordinal 1=A..5=E) at each window timepoint, i…
- L717 `predict_multistate(X)` — AIS multi-state recovery for one admission row (see header).
- L777 `_independence_input(X)` — One-row input over the independence bundle's feature universe, dtyped like train…
- L792 `predict_independence(X)` — Per-SCIM-item discharge functional-independence profile for one admission row.
- L832 `independence_observed_for_episode(key_record)` — Realized discharge functional independence per item for one episode (discharge i…
...
- L26 `topbar(lang)`
- L54 `kpi_card(label, value, sub)`
- L61 `chart_card(title, content)`
- L66 `input_id(prefix, col)`
- L70 `number_input_for(feature, lang, defaults)` — Clearable numeric input. A blank field is left unknown (NaN) so the model
- L102 `dropdown_for(feature, lang, defaults)`
- L131 `fig_shap_local(values, X, base, lang)`
- L160 `fig_prediction_interval(pred, lo, hi, spec, lang)`
- L194 `fig_class_probabilities(proba, class_labels, spec, lang, conformal_set)`
- L230 `fig_landmark_compare(result, spec, lang, landmark_label)` — Paired admission-only vs landmark prediction for one outcome (see compute.predic…
- L298 `landmark_readout(result, spec, lang)` — Two-line baseline→landmark summary shared by the simulator and patient dynamic c…
- L333 `_voi_label(measure, lang)`
- L337 `fig_voi_patient(voi, spec, lang)` — Per-patient value-of-information bars (see compute.landmark_voi).
- L403 `voi_readout(voi, spec, lang)` — One/two-line prescription: the most valuable next measure to obtain (+ best alre…
- L445 `_conversion_endpoint_label(key, letter, lang)` — Clinical endpoint name + its discharge threshold, e.g. 'Motor-incomplete (≥C)'.
- L450 `fig_conversion_endpoints(result, lang)` — Calibrated conversion probabilities for the applicable binary endpoints (see
- L487 `fig_conversion_magnitude(result, lang)` — Ordinal improvement-magnitude head shown AS its 80% APS set / most-likely class …
- L521 `conversion_readout(result, lang)` — Text summary of the conversion panel: admission grade, each applicable calibrate…
- L568 `_ms_target_curves(result)` — Map the available first-passage curves to DISTINCT target grades.  ``improve`` i…
- L585 `fig_multistate_trajectory(result, lang, patient_obs)` — Expected AIS-grade trajectory for the admission grade's cohort (occupancy-weight…
- L632 `fig_multistate_conversion_personal(result, lang)` — First-passage conversion curves for the admission grade: P(reach each higher AIS…
- L661 `multistate_readout(result, lang)` — Text summary of the multi-state panel: admission grade, the calibrated improve-b…
- L705 `fig_independence_profile(result, lang, observed)` — Per-SCIM-item discharge functional-independence profile: one horizontal bar = ca…
- L769 `independence_readout(result, lang)` — Text summary of the independence profile: the expected number of independent fun…
...
- L166 `PATIENT_OPTIONS` (const)
- L167 `PATIENT_OPTIONS_BY_ID` (const)
...
- L55 `INK` (const)
- L67 `apply_template()` — Register and activate the medical-grade plotly template.
...
### methods.py (1000 lines)
Plotly figures for the Methods tab — calibration and performance visualizations.
- L19 `fig_pred_vs_observed(shap_pack, schema, lang, *, clip_min, clip_max, axis_label)`
- L86 `fig_residual_hist(shap_pack, schema, lang, *, axis_label)`
- L127 `fig_confusion_matrix(shap_pack, schema, lang)`
- L171 `fig_calibration_curve(shap_pack, schema, lang, *, n_bins)`
- L232 `fig_dataquality_overview(summary, lang)` — Stacked bar of finding counts per category, split by severity.
- L276 `fig_temporal_drift(t_outcome, lang)` — Out-of-time drift across rolling-origin test years (F24).
- L352 `fig_landmark_value(lm_outcome, landmark_days, lang)` — Value of observation: discharge-outcome accuracy + PI sharpening vs landmark tim…
- L427 `fig_voi_scorecard(lm_outcome, lang, measure_labels)` — Value-of-information scorecard: per-measure × per-landmark uncertainty reduction…
- L488 `fig_conversion_landscape(conv, lang)` — Descriptive conversion landscape: ≥1-grade AIS improvement rate by admission gra…
- L519 `fig_conversion_delta(conv, lang)` — Distribution of the AIS grade change (discharge − admission) over the dual-AIS c…
- L547 `fig_conversion_reliability(em, lang, label)` — Reliability curve for one binary conversion endpoint: Platt-calibrated vs raw Li…
- L592 `fig_conversion_shap(em, schema, lang, top_n)` — Descriptive in-sample SHAP drivers for one conversion endpoint (mean |SHAP| on t…
- L614 `fig_conversion_confusion(mag, lang)` — Row-normalized confusion matrix for the ordinal magnitude head over {0, +1, ≥+2}…
- L645 `_ms_xlabels(ms, schema, lang)`
- L649 `fig_multistate_occupancy(ms, schema, lang)` — State-occupancy (prevalence) curves P(in AIS state g at time t), as a stacked ar…
- L679 `fig_multistate_conversion(ms, schema, lang)` — First-passage conversion curves P(reached threshold by time t) from each admissi…
- L715 `fig_multistate_transition(ms, lang)` — Pooled one-step AIS transition matrix (time-averaged over the grid).  Rows = fro…
- L744 `fig_multistate_sojourn(ms, lang)` — Expected days spent in each AIS state over the 0day-6m window, per admission gra…
- L768 `fig_multistate_improve_base(ms, lang)` — Observed ≥1-grade improvement rate by admission grade — the non-monotone base ra…
- L800 `_ind_modeled_items(ind)` — Registry items that have a fitted head, in display (domain-grouped) order.
- L806 `_ind_domain_order(items)` — Unique SCIM domains in registry order.
- L815 `_ind_domain_color(dom)`
- L819 `fig_independence_scorecard(ind, schema, lang)` — Per-item discrimination + calibration scorecard: AUC (left) and Brier skill scor…
- L870 `fig_independence_calibration(ind, schema, lang)` — All-heads reliability overlay: every item's Platt-calibrated curve against the d…
- L910 `fig_independence_shap_heatmap(ind, schema, lang, top_features)` — Item x driver heatmap: for each item, the mean |SHAP| of the globally most-impor…
- L951 `fig_independence_landscape(ind, schema, lang)` — Item x admission-AIS independence-rate landscape: observed P(independent at disc…
...
### methods.py (661 lines)
Methods tab — model documentation + per-outcome performance visualizations.
- L26 `_perf_block_regression(spec, info, lang)`
- L88 `_perf_block_multiclass(spec, info, lang)`
- L157 `_perf_block_trajectory(lang)`
- L184 `_temporal_block(lang)` — F24 — out-of-time rolling-origin drift, one card per outcome.
- L234 `_landmark_block(lang)` — G1 — landmark (dynamic) prediction: value-of-observation curve, one card per out…
- L296 `_conversion_endpoint_label(key, discharge_min, lang)`
- L300 `_conversion_endpoint_card(key, em, lang)` — One binary endpoint: headline metrics + base-rate-by-grade table + reliability/S…
- L337 `_conversion_block(lang)` — G4 — AIS-grade conversion: descriptive landscape + per-endpoint calibration + ma…
- L407 `_multistate_block(lang)` — G6 — AIS multi-state recovery: cohort dynamics (occupancy / first-passage / tran…
- L476 `_independence_block(lang)` — G7 — functional-independence profile: per-item scorecard + admission-AIS landsca…
- L549 `update_methods_independence_item(item_key, lang)` [callback] — Per-item drilldown: the selected independence head's raw-vs-Platt reliability cu…
- L563 `_dataquality_block(lang)`
- L611 `render_methods(lang)`
...
### patient.py (1063 lines)
Patient explorer tab — real-patient predictions, similarity, PDF report.
- L88 `_patient_picker_options(lang)`
- L111 `_episode_options_for_patient(id_number, lang)`
- L121 `_meta_strip(meta, lang)`
- L174 `_isncsci_table(long_df, key_record, lang)`
- L217 `_landmark_obs_note(observed, landmark, lang)` — One-line summary of the real early-recovery scores feeding the landmark predicti…
- L232 `_patient_landmark_card(lang)` — Real-data dynamic-prediction card: at a chosen landmark the patient's own observ…
- L269 `_phenotype_readout(res, lang)` — Dominant phenotype + membership-weighted conditioned prognosis for one patient.
- L306 `_patient_phenotype_card(lang)` — Observed-trajectory phenotype card: the patient's own early SCIM/motor curve is …
- L337 `_patient_conversion_card(lang)` — AIS-grade conversion card: the patient's admission row drives the calibrated end…
- L357 `_patient_multistate_card(lang)` — AIS multi-state recovery card: the patient's admission grade drives the cohort m…
- L379 `_patient_independence_card(lang)` — Functional-independence profile card: the patient's admission row drives the cal…
- L399 `render_patient(lang)`
- L518 `_patient_regression(bundle, X, key_record, lang)`
- L574 `_patient_multiclass(bundle, X, key_record, lang)`
- L616 `_build_similarity_section(key_record, bundle, X, lang)`
- L705 `_compute_patient_tab(key_record, strata, outcome_key, lang)`
- L772 `update_patient_picker(id_number, lang)` [callback]
- L781 `reset_episode_on_patient_change(id_number, current)` [callback]
- L805 `update_patient_tab(key_record, strata, outcome_key, lang)` [callback]
- L815 `update_patient_landmark_options(key_record)` [callback] — Offer only the landmarks this episode is still-admitted-eligible for; default to…
- L835 `update_patient_landmark(landmark, key_record, outcome_key, lang)` [callback]
- L868 `update_patient_phenotype_options(key_record, lang)` [callback] — Offer each observation-cutoff this episode is eligible for; default to the full …
- L885 `update_patient_phenotype(cutoff, key_record, lang)` [callback]
- L913 `update_patient_conversion(key_record, lang)` [callback]
- L945 `update_patient_multistate(key_record, lang)` [callback]
- L975 `update_patient_independence(key_record, lang)` [callback]
- L998 `download_report(n_clicks, key_record, id_number, strata, lang)` [callback]

### simulator.py (716 lines)
Simulator tab — hypothetical patient prediction + What-if counterfactual.
- L69 `render_simulator(lang, ref_data)`
- L184 `_conversion_card(lang)` — Hypothetical AIS-grade conversion card driven by the simulator's admission input…
- L206 `_multistate_card(lang)` — Hypothetical AIS multi-state recovery card driven by the simulator's admission i…
- L230 `_independence_card(lang)` — Hypothetical functional-independence profile driven by the simulator's admission…
- L250 `_lm_obs_input(measure, lang)`
- L263 `_landmark_card(lang)` — Hypothetical dynamic-prediction card: pick a landmark, enter observed scores, se…
- L300 `_simulate_regression(bundle, X, lang)`
- L335 `_simulate_multiclass(bundle, X, lang)`
- L364 `_reliability_badge(a, lang)`
- L427 `simulate(num_vals, cat_vals, num_ids, cat_ids, outcome_key, lang, ref_data)` [callback]
- L515 `launch_whatif(n_clicks, key_record, id_number)` [callback]
- L564 `update_whatif_banner(ref_data, lang)` [callback]
- L588 `clear_whatif(_n)` [callback]
- L602 `fill_or_clear(_fill, _clear, num_ids, cat_ids)` [callback] — Fill every field with the cohort default, or clear all to blank (NaN).
- L626 `simulate_landmark(landmark, obs_vals, num_vals, cat_vals, outcome_key, lang, obs_ids, num_ids, cat_ids)` [callback]
- L657 `simulate_conversion(num_vals, cat_vals, lang, num_ids, cat_ids)` [callback]
- L682 `simulate_multistate(num_vals, cat_vals, lang, num_ids, cat_ids)` [callback]
- L707 `simulate_independence(num_vals, cat_vals, lang, num_ids, cat_ids)` [callback]
