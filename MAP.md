...
### compute.py (1283 lines)
Pure computation helpers for model inference, conformal PI, and SHAP.
- L42 `resolve_group_q(q_by_group, marginal, X)` — Resolve Mondrian q for a single-row input.
- L65 `resolve_conformal_q(fspec, X)`
- L73 `resolve_aps_q(fspec, X)`
- L82 `predict_trajectory(X)` — Predict SCIM-total at each trajectory timepoint for a single-row input.
- L119 `aps_prediction_set(proba_row, q_hat)`
- L132 `inv_transform_scalar(x, transform)`
- L138 `clip_scalar(x, lo, hi)`
- L146 `format_value(col, value)`
- L155 `compute_ref_predictions(X)` — Compute predictions for all outcomes on a single-row X.
- L194 `collect_sim_inputs(num_vals, num_ids, cat_vals, cat_ids)`
- L213 `shap_for_row_regression(X, model)`
- L226 `shap_for_row_class(X, clf, class_idx, n_classes)`
- L255 `episode_row_for_model(key_record)` — Build a one-row model input from an episode's admission features.
- L271 `episode_has_admission(key_record)`
- L276 `get_observed_for_outcome(key_record, spec)`
- L289 `_landmark_input(X_base, observed, feature_cols)` — Build a one-row model input over ``feature_cols`` from base features + observed …
- L312 `_predict_landmark_head(head, X, task, transform, cmin, cmax)`
- L339 `predict_landmark(outcome_key, landmark, X_base, observed)` — Paired admission-only baseline vs landmark prediction for one outcome at landmar…
- L366 `landmark_voi(outcome_key, landmark, X_base, observed)` — Per-measure value-of-information for one patient at landmark ``L`` (G2).
- L436 `_episode_timepoint_oidx(key_record)` — Episode rows restricted to the landmark measures, plus the timepoint order-index…
- L446 `landmark_observed_for_episode(key_record, landmark)` — Real LOCF observed block for one episode: last non-null value of each landmark m…
- L465 `episode_landmark_eligibility(key_record)` — Per-landmark still-admitted eligibility: True when the episode has a tracked obs…
- L482 `_phenotype_episode_obs(key_record)` — This episode's observed ``(timepoint, value)`` pairs within the phenotyping wind…
- L498 `phenotype_cutoff_options(key_record, min_cells)` — Window timepoints (chronological) eligible as observation-cutoffs for phenotype
- L522 `predict_phenotype_membership(key_record, cutoff)` — Soft phenotype membership for one episode using only observations on/before ``cu…
- L592 `_apply_platt(calibrator, prob)` — Platt (sigmoid) recalibration over the LightGBM logit — mirrors models.conversio…
- L599 `_conversion_input(X)` — One-row model input over the conversion bundle's feature universe, dtyped like t…
- L614 `mag_short_label(code, mag_cap)` — Compact, language-neutral label for an ordinal improvement-magnitude class.
- L623 `predict_conversion(X)` — Admission->discharge AIS conversion for one admission row (see header).
- L691 `_multistate_input(X)` — One-row improve-head input over the multi-state bundle's feature universe, dtype…
- L706 `multistate_observed_grades(key_record)` — This episode's observed AIS grade (ordinal 1=A..5=E) at each window timepoint, i…
- L722 `predict_multistate(X)` — AIS multi-state recovery for one admission row (see header).
- L782 `_independence_input(X)` — One-row input over the independence bundle's feature universe, dtyped like train…
- L797 `predict_independence(X)` — Per-SCIM-item discharge functional-independence profile for one admission row.
- L837 `independence_observed_for_episode(key_record)` — Realized discharge functional independence per item for one episode (discharge i…
- L870 `_topography_input(X)` — One-row input over the 30 shared admission features, dtyped like training; every…
- L885 `topography_admission_grades(key_record)` — The patient's real per-segment admission ISNCSCI grades: first non-null over the
- L911 `predict_topography(X, adm_grades)` — Per-ISNCSCI-segment discharge functional-milestone probabilities for one admissi…
- L954 `topography_cohort_atlas()` — Cohort base-rate atlas in the predict_topography result shape: every segment's `…
- L975 `topography_observed_discharge(key_record)` — Realized discharge milestone per segment (discharge grade >= the segment thresho…
- L1009 `_level_descent_input(X)` — One-row input over the bundle's 30 admission features, dtyped like training (eve…
- L1025 `predict_level_descent(X)` — Per-level admission->discharge neurological-level descent for one admission row …
- L1071 `level_descent_observed(key_record)` — The patient's OWN admission->discharge level change per modelled level, for the …
- L1137 `_dissociation_input(X)` — One-row input over the dissociation bundle's feature universe (the shared 30 adm…
- L1152 `predict_dissociation(X)` — Per-axis neuro-functional dissociation for one admission row (see header).
- L1204 `_axis_cohort_deltas(meta)` — (Δneuro, Δfunction) over the whole episode frame for one axis (NaN where undefin…
- L1214 `dissociation_cohort_landscape()` — Per-axis cohort scatter recomputed live from the episode frame (only compact sum…
- L1255 `dissociation_observed(key_record)` — The patient's OWN observed (Δneuro, Δfunction, D) per axis for the patient-card …
...
- L574 `_ld_label(level, lang)`
- L578 `fig_level_descent_bar(result, lang)` — Calibrated descent probabilities P(Δ≥1 caudal level) for the applicable levels (…
- L616 `fig_level_descent_magnitude(result, lang)` — Ordinal descent-magnitude heads, surfaced AS their uncalibrated relative likelih…
- L657 `level_descent_readout(result, lang)` — Text summary of the level-descent panel: per applicable level, the calibrated de…
- L684 `fig_level_descent_observed(observed, lang)` — The patient's OWN admission->discharge level change per modelled level (see
- L737 `_ms_target_curves(result)` — Map the available first-passage curves to DISTINCT target grades.  ``improve`` i…
- L754 `fig_multistate_trajectory(result, lang, patient_obs)` — Expected AIS-grade trajectory for the admission grade's cohort (occupancy-weight…
- L801 `fig_multistate_conversion_personal(result, lang)` — First-passage conversion curves for the admission grade: P(reach each higher AIS…
- L830 `multistate_readout(result, lang)` — Text summary of the multi-state panel: admission grade, the calibrated improve-b…
- L874 `fig_independence_profile(result, lang, observed)` — Per-SCIM-item discharge functional-independence profile: one horizontal bar = ca…
- L938 `independence_readout(result, lang)` — Text summary of the independence profile: the expected number of independent fun…
- L994 `_poly_path(pts)` — SVG path string for a closed polygon from (x, y) vertices.
- L1001 `_topo_body_shapes()` — Stylised front-view humanoid silhouette as a list of add_shape kwargs (data coor…
- L1021 `_topo_segment_lookup(result, modality)` — {(level, side): segment-record} for one modality from a predict_topography resul…
- L1026 `fig_topography_bodymap(result, lang, sensory_modality, observed, title)` — The recovery-topography atlas: a stylised front-view dermatome silhouette (the c…
- L1161 `topography_readout(result, lang)` — Text summary of the topography atlas: expected count of muscles reaching antigra…
- L1210 `_diss_short(key, lang)`
- L1214 `fig_dissociation_landscape(cohort, lang, predict, observed)` — Per-axis Δneuro×Δfunction quadrant scatters (one small-multiple per domain-paire…
- L1308 `dissociation_readout(result, lang)` — Text summary of the dissociation prediction: per axis the calibrated direction (…
...
- L196 `LEVEL_DESCENT_BUNDLE` (const)
- L213 `DISSOCIATION_BUNDLE` (const)
- L217 `PATIENT_OPTIONS` (const)
- L218 `PATIENT_OPTIONS_BY_ID` (const)
...
- L650 `_ld_levels(ld, lang)` — (keys-in-display-order, matching row labels) for the modelled levels present in …
- L658 `fig_level_descent_scorecard(ld, lang)` — Per-level descent discrimination: horizontal OOF AUC bars over the five modelled…
- L691 `fig_level_descent_landscape(ld, lang)` — Descriptive per-level outcome composition: a stacked horizontal bar of deteriora…
- L731 `fig_level_descent_delta(landscape, lang)` — Distribution of one level's change Δ (discharge − admission cord ordinal) for th…
- L759 `_ms_xlabels(ms, schema, lang)`
- L763 `fig_multistate_occupancy(ms, schema, lang)` — State-occupancy (prevalence) curves P(in AIS state g at time t), as a stacked ar…
- L793 `fig_multistate_conversion(ms, schema, lang)` — First-passage conversion curves P(reached threshold by time t) from each admissi…
- L829 `fig_multistate_transition(ms, lang)` — Pooled one-step AIS transition matrix (time-averaged over the grid).  Rows = fro…
- L858 `fig_multistate_sojourn(ms, lang)` — Expected days spent in each AIS state over the 0day-6m window, per admission gra…
- L882 `fig_multistate_improve_base(ms, lang)` — Observed ≥1-grade improvement rate by admission grade — the non-monotone base ra…
- L914 `_ind_modeled_items(ind)` — Registry items that have a fitted head, in display (domain-grouped) order.
- L920 `_ind_domain_order(items)` — Unique SCIM domains in registry order.
- L929 `_ind_domain_color(dom)`
- L933 `fig_independence_scorecard(ind, schema, lang)` — Per-item discrimination + calibration scorecard: AUC (left) and Brier skill scor…
- L984 `fig_independence_calibration(ind, schema, lang)` — All-heads reliability overlay: every item's Platt-calibrated curve against the d…
- L1024 `fig_independence_shap_heatmap(ind, schema, lang, top_features)` — Item x driver heatmap: for each item, the mean |SHAP| of the globally most-impor…
- L1065 `fig_independence_landscape(ind, schema, lang)` — Item x admission-AIS independence-rate landscape: observed P(independent at disc…
- L1126 `_topo_short_modality(mod, lang)`
- L1132 `_topo_modelable(topo)` — Non-degenerate per-segment metric records (those carrying auc/calibration/shap_t…
- L1137 `fig_topography_calibration(topo, lang)` — Pooled per-modality reliability: each modality's Platt-calibrated curve (all its…
- L1171 `fig_topography_scorecard(topo, lang)` — Per-segment discrimination spread by modality: a horizontal box of the per-segme…
- L1209 `fig_topography_drivers(topo, schema, lang)` — Per-modality SHAP drivers: grouped horizontal bars of the top admission features…
- L1240 `_diss_axis_label(em, lang)` — Compact axis label for the scorecard, derived from the metrics' verbose neuro/fu…
- L1246 `fig_dissociation_scorecard(diss, lang)` — Per-axis predictability of the dissociation from admission features: the calibra…
...
### methods.py (951 lines)
Methods tab — model documentation + per-outcome performance visualizations.
- L35 `_perf_block_regression(spec, info, lang)`
- L97 `_perf_block_multiclass(spec, info, lang)`
- L166 `_perf_block_trajectory(lang)`
- L193 `_temporal_block(lang)` — F24 — out-of-time rolling-origin drift, one card per outcome.
- L243 `_landmark_block(lang)` — G1 — landmark (dynamic) prediction: value-of-observation curve, one card per out…
- L305 `_conversion_endpoint_label(key, discharge_min, lang)`
- L309 `_conversion_endpoint_card(key, em, lang)` — One binary endpoint: headline metrics + base-rate-by-grade table + reliability/S…
- L346 `_conversion_block(lang)` — G4 — AIS-grade conversion: descriptive landscape + per-endpoint calibration + ma…
- L416 `_multistate_block(lang)` — G6 — AIS multi-state recovery: cohort dynamics (occupancy / first-passage / tran…
- L485 `_independence_block(lang)` — G7 — functional-independence profile: per-item scorecard + admission-AIS landsca…
- L558 `update_methods_independence_item(item_key, lang)` [callback] — Per-item drilldown: the selected independence head's raw-vs-Platt reliability cu…
- L572 `_topo_seg_label(entry, lang)` — Compact 'modality · side · level' label for one topography segment.
- L580 `_topography_block(lang)` — G8 — recovery topography map: the cohort body-map atlas (modality-toggleable der…
- L662 `update_methods_topography_atlas(modality, lang)` [callback] — Cohort recovery-topography atlas: the body map colored by each segment's observe…
- L677 `update_methods_topography_segment(seg_key, lang)` [callback] — Per-segment drilldown: the selected segment head's raw-vs-Platt reliability curv…
- L694 `_ld_level_label(key, lang)`
- L699 `_level_descent_block(lang)` — G10 — neurological-level descent: per-level descent-discrimination scorecard + o…
- L759 `update_methods_level_descent(level_key, lang)` [callback] — Per-level drilldown: the selected level's descent-head raw-vs-Platt reliability …
- L776 `_dataquality_block(lang)`
- L824 `_dissociation_block(lang)` — G11 — neuro-functional dissociation: the cohort Δneuro×Δfunction quadrant map (p…
- L877 `update_methods_dissociation_axis(axis_key, lang)` [callback] — Per-axis drilldown: the selected axis's over-achiever (binary) head raw-vs-Platt…
- L892 `render_methods(lang)`
...
### patient.py (1245 lines)
Patient explorer tab — real-patient predictions, similarity, PDF report.
- L107 `_patient_picker_options(lang)`
- L130 `_episode_options_for_patient(id_number, lang)`
- L140 `_meta_strip(meta, lang)`
- L193 `_isncsci_table(long_df, key_record, lang)`
- L236 `_landmark_obs_note(observed, landmark, lang)` — One-line summary of the real early-recovery scores feeding the landmark predicti…
- L251 `_patient_landmark_card(lang)` — Real-data dynamic-prediction card: at a chosen landmark the patient's own observ…
- L288 `_phenotype_readout(res, lang)` — Dominant phenotype + membership-weighted conditioned prognosis for one patient.
- L325 `_patient_phenotype_card(lang)` — Observed-trajectory phenotype card: the patient's own early SCIM/motor curve is …
- L356 `_patient_conversion_card(lang)` — AIS-grade conversion card: the patient's admission row drives the calibrated end…
- L376 `_patient_multistate_card(lang)` — AIS multi-state recovery card: the patient's admission grade drives the cohort m…
- L398 `_patient_independence_card(lang)` — Functional-independence profile card: the patient's admission row drives the cal…
- L417 `_patient_topography_card(lang)` — Recovery-topography card: the patient's real admission ISNCSCI exam (per-segment…
- L442 `_patient_level_descent_card(lang)` — Neurological-level descent card: the patient's REAL admission levels (per-level …
- L467 `_patient_dissociation_card(lang)` — Neuro-functional dissociation card: the patient's admission row drives the predi…
- L487 `render_patient(lang)`
- L615 `_patient_regression(bundle, X, key_record, lang)`
- L671 `_patient_multiclass(bundle, X, key_record, lang)`
- L713 `_build_similarity_section(key_record, bundle, X, lang)`
- L802 `_compute_patient_tab(key_record, strata, outcome_key, lang)`
- L869 `update_patient_picker(id_number, lang)` [callback]
- L878 `reset_episode_on_patient_change(id_number, current)` [callback]
- L902 `update_patient_tab(key_record, strata, outcome_key, lang)` [callback]
- L912 `update_patient_landmark_options(key_record)` [callback] — Offer only the landmarks this episode is still-admitted-eligible for; default to…
- L932 `update_patient_landmark(landmark, key_record, outcome_key, lang)` [callback]
- L965 `update_patient_phenotype_options(key_record, lang)` [callback] — Offer each observation-cutoff this episode is eligible for; default to the full …
- L982 `update_patient_phenotype(cutoff, key_record, lang)` [callback]
- L1010 `update_patient_conversion(key_record, lang)` [callback]
- L1042 `update_patient_multistate(key_record, lang)` [callback]
- L1072 `update_patient_independence(key_record, lang)` [callback]
- L1093 `update_patient_topography(key_record, modality, lang)` [callback]
- L1120 `update_patient_level_descent(key_record, lang)` [callback]
- L1154 `update_patient_dissociation(key_record, lang)` [callback]
- L1180 `download_report(n_clicks, key_record, id_number, strata, lang)` [callback]

### simulator.py (974 lines)
Simulator tab — hypothetical patient prediction + What-if counterfactual.
- L84 `render_simulator(lang, ref_data)`
- L208 `_conversion_card(lang)` — Hypothetical AIS-grade conversion card driven by the simulator's admission input…
- L230 `_level_descent_card(lang)` — Hypothetical neurological-level descent card driven by the simulator's admission…
- L254 `_multistate_card(lang)` — Hypothetical AIS multi-state recovery card driven by the simulator's admission i…
- L278 `_independence_card(lang)` — Hypothetical functional-independence profile driven by the simulator's admission…
- L297 `_dissociation_card(lang)` — Hypothetical neuro-functional dissociation driven by the simulator's admission i…
- L317 `_topo_worksheet(lang, seed)` — ISNCSCI-worksheet-style editable admission-grade grid: rows = cord levels (rostr…
- L355 `_topography_card(lang, ref_data)` — Hypothetical recovery-topography card: an editable per-segment admission workshe…
- L394 `_lm_obs_input(measure, lang)`
- L407 `_landmark_card(lang)` — Hypothetical dynamic-prediction card: pick a landmark, enter observed scores, se…
- L444 `_simulate_regression(bundle, X, lang)`
- L488 `_simulate_multiclass(bundle, X, lang)`
- L517 `_reliability_badge(a, lang)`
- L580 `simulate(num_vals, cat_vals, num_ids, cat_ids, outcome_key, lang, ref_data)` [callback]
- L668 `launch_whatif(n_clicks, key_record, id_number)` [callback]
- L717 `update_whatif_banner(ref_data, lang)` [callback]
- L741 `clear_whatif(_n)` [callback]
- L755 `fill_or_clear(_fill, _clear, num_ids, cat_ids)` [callback] — Fill every field with the cohort default, or clear all to blank (NaN).
- L779 `simulate_landmark(landmark, obs_vals, num_vals, cat_vals, outcome_key, lang, obs_ids, num_ids, cat_ids)` [callback]
- L810 `simulate_conversion(num_vals, cat_vals, lang, num_ids, cat_ids)` [callback]
- L835 `simulate_level_descent(num_vals, cat_vals, lang, num_ids, cat_ids)` [callback]
- L863 `simulate_multistate(num_vals, cat_vals, lang, num_ids, cat_ids)` [callback]
- L888 `simulate_independence(num_vals, cat_vals, lang, num_ids, cat_ids)` [callback]
- L910 `simulate_dissociation(num_vals, cat_vals, lang, num_ids, cat_ids)` [callback]
- L934 `topo_seed_or_clear(_seed, _clear, seg_ids, ref_data)` [callback] — Seed the worksheet from the What-if reference patient's real admission exam, or …
- L956 `simulate_topography(num_vals, cat_vals, seg_vals, modality, lang, num_ids, cat_ids, seg_ids)` [callback]
