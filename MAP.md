...
### compute.py (1112 lines)
Pure computation helpers for model inference, conformal PI, and SHAP.
- L41 `resolve_group_q(q_by_group, marginal, X)` — Resolve Mondrian q for a single-row input.
- L64 `resolve_conformal_q(fspec, X)`
- L72 `resolve_aps_q(fspec, X)`
- L81 `predict_trajectory(X)` — Predict SCIM-total at each trajectory timepoint for a single-row input.
- L118 `aps_prediction_set(proba_row, q_hat)`
- L131 `inv_transform_scalar(x, transform)`
- L137 `clip_scalar(x, lo, hi)`
- L145 `format_value(col, value)`
- L154 `compute_ref_predictions(X)` — Compute predictions for all outcomes on a single-row X.
- L193 `collect_sim_inputs(num_vals, num_ids, cat_vals, cat_ids)`
- L212 `shap_for_row_regression(X, model)`
- L225 `shap_for_row_class(X, clf, class_idx, n_classes)`
- L254 `episode_row_for_model(key_record)` — Build a one-row model input from an episode's admission features.
- L270 `episode_has_admission(key_record)`
- L275 `get_observed_for_outcome(key_record, spec)`
- L288 `_landmark_input(X_base, observed, feature_cols)` — Build a one-row model input over ``feature_cols`` from base features + observed …
- L311 `_predict_landmark_head(head, X, task, transform, cmin, cmax)`
- L338 `predict_landmark(outcome_key, landmark, X_base, observed)` — Paired admission-only baseline vs landmark prediction for one outcome at landmar…
- L365 `landmark_voi(outcome_key, landmark, X_base, observed)` — Per-measure value-of-information for one patient at landmark ``L`` (G2).
- L435 `_episode_timepoint_oidx(key_record)` — Episode rows restricted to the landmark measures, plus the timepoint order-index…
- L445 `landmark_observed_for_episode(key_record, landmark)` — Real LOCF observed block for one episode: last non-null value of each landmark m…
- L464 `episode_landmark_eligibility(key_record)` — Per-landmark still-admitted eligibility: True when the episode has a tracked obs…
- L481 `_phenotype_episode_obs(key_record)` — This episode's observed ``(timepoint, value)`` pairs within the phenotyping wind…
- L497 `phenotype_cutoff_options(key_record, min_cells)` — Window timepoints (chronological) eligible as observation-cutoffs for phenotype
- L521 `predict_phenotype_membership(key_record, cutoff)` — Soft phenotype membership for one episode using only observations on/before ``cu…
- L591 `_apply_platt(calibrator, prob)` — Platt (sigmoid) recalibration over the LightGBM logit — mirrors models.conversio…
- L598 `_conversion_input(X)` — One-row model input over the conversion bundle's feature universe, dtyped like t…
- L613 `mag_short_label(code, mag_cap)` — Compact, language-neutral label for an ordinal improvement-magnitude class.
- L622 `predict_conversion(X)` — Admission->discharge AIS conversion for one admission row (see header).
- L690 `_multistate_input(X)` — One-row improve-head input over the multi-state bundle's feature universe, dtype…
- L705 `multistate_observed_grades(key_record)` — This episode's observed AIS grade (ordinal 1=A..5=E) at each window timepoint, i…
- L721 `predict_multistate(X)` — AIS multi-state recovery for one admission row (see header).
- L781 `_independence_input(X)` — One-row input over the independence bundle's feature universe, dtyped like train…
- L796 `predict_independence(X)` — Per-SCIM-item discharge functional-independence profile for one admission row.
- L836 `independence_observed_for_episode(key_record)` — Realized discharge functional independence per item for one episode (discharge i…
- L869 `_topography_input(X)` — One-row input over the 30 shared admission features, dtyped like training; every…
- L884 `topography_admission_grades(key_record)` — The patient's real per-segment admission ISNCSCI grades: first non-null over the
- L910 `predict_topography(X, adm_grades)` — Per-ISNCSCI-segment discharge functional-milestone probabilities for one admissi…
- L953 `topography_cohort_atlas()` — Cohort base-rate atlas in the predict_topography result shape: every segment's `…
- L974 `topography_observed_discharge(key_record)` — Realized discharge milestone per segment (discharge grade >= the segment thresho…
- L1008 `_level_descent_input(X)` — One-row input over the bundle's 30 admission features, dtyped like training (eve…
- L1024 `predict_level_descent(X)` — Per-level admission->discharge neurological-level descent for one admission row …
- L1070 `level_descent_observed(key_record)` — The patient's OWN admission->discharge level change per modelled level, for the …
...
- L200 `PATIENT_OPTIONS` (const)
- L201 `PATIENT_OPTIONS_BY_ID` (const)
...
### methods.py (875 lines)
Methods tab — model documentation + per-outcome performance visualizations.
- L30 `_perf_block_regression(spec, info, lang)`
- L92 `_perf_block_multiclass(spec, info, lang)`
- L161 `_perf_block_trajectory(lang)`
- L188 `_temporal_block(lang)` — F24 — out-of-time rolling-origin drift, one card per outcome.
- L238 `_landmark_block(lang)` — G1 — landmark (dynamic) prediction: value-of-observation curve, one card per out…
- L300 `_conversion_endpoint_label(key, discharge_min, lang)`
- L304 `_conversion_endpoint_card(key, em, lang)` — One binary endpoint: headline metrics + base-rate-by-grade table + reliability/S…
- L341 `_conversion_block(lang)` — G4 — AIS-grade conversion: descriptive landscape + per-endpoint calibration + ma…
- L411 `_multistate_block(lang)` — G6 — AIS multi-state recovery: cohort dynamics (occupancy / first-passage / tran…
- L480 `_independence_block(lang)` — G7 — functional-independence profile: per-item scorecard + admission-AIS landsca…
- L553 `update_methods_independence_item(item_key, lang)` [callback] — Per-item drilldown: the selected independence head's raw-vs-Platt reliability cu…
- L567 `_topo_seg_label(entry, lang)` — Compact 'modality · side · level' label for one topography segment.
- L575 `_topography_block(lang)` — G8 — recovery topography map: the cohort body-map atlas (modality-toggleable der…
- L657 `update_methods_topography_atlas(modality, lang)` [callback] — Cohort recovery-topography atlas: the body map colored by each segment's observe…
- L672 `update_methods_topography_segment(seg_key, lang)` [callback] — Per-segment drilldown: the selected segment head's raw-vs-Platt reliability curv…
- L689 `_ld_level_label(key, lang)`
- L694 `_level_descent_block(lang)` — G10 — neurological-level descent: per-level descent-discrimination scorecard + o…
- L754 `update_methods_level_descent(level_key, lang)` [callback] — Per-level drilldown: the selected level's descent-head raw-vs-Platt reliability …
- L771 `_dataquality_block(lang)`
- L819 `render_methods(lang)`
...
### patient.py (1194 lines)
Patient explorer tab — real-patient predictions, similarity, PDF report.
- L101 `_patient_picker_options(lang)`
- L124 `_episode_options_for_patient(id_number, lang)`
- L134 `_meta_strip(meta, lang)`
- L187 `_isncsci_table(long_df, key_record, lang)`
- L230 `_landmark_obs_note(observed, landmark, lang)` — One-line summary of the real early-recovery scores feeding the landmark predicti…
- L245 `_patient_landmark_card(lang)` — Real-data dynamic-prediction card: at a chosen landmark the patient's own observ…
- L282 `_phenotype_readout(res, lang)` — Dominant phenotype + membership-weighted conditioned prognosis for one patient.
- L319 `_patient_phenotype_card(lang)` — Observed-trajectory phenotype card: the patient's own early SCIM/motor curve is …
- L350 `_patient_conversion_card(lang)` — AIS-grade conversion card: the patient's admission row drives the calibrated end…
- L370 `_patient_multistate_card(lang)` — AIS multi-state recovery card: the patient's admission grade drives the cohort m…
- L392 `_patient_independence_card(lang)` — Functional-independence profile card: the patient's admission row drives the cal…
- L411 `_patient_topography_card(lang)` — Recovery-topography card: the patient's real admission ISNCSCI exam (per-segment…
- L436 `_patient_level_descent_card(lang)` — Neurological-level descent card: the patient's REAL admission levels (per-level …
- L462 `render_patient(lang)`
- L587 `_patient_regression(bundle, X, key_record, lang)`
- L643 `_patient_multiclass(bundle, X, key_record, lang)`
- L685 `_build_similarity_section(key_record, bundle, X, lang)`
- L774 `_compute_patient_tab(key_record, strata, outcome_key, lang)`
- L841 `update_patient_picker(id_number, lang)` [callback]
- L850 `reset_episode_on_patient_change(id_number, current)` [callback]
- L874 `update_patient_tab(key_record, strata, outcome_key, lang)` [callback]
- L884 `update_patient_landmark_options(key_record)` [callback] — Offer only the landmarks this episode is still-admitted-eligible for; default to…
- L904 `update_patient_landmark(landmark, key_record, outcome_key, lang)` [callback]
- L937 `update_patient_phenotype_options(key_record, lang)` [callback] — Offer each observation-cutoff this episode is eligible for; default to the full …
- L954 `update_patient_phenotype(cutoff, key_record, lang)` [callback]
- L982 `update_patient_conversion(key_record, lang)` [callback]
- L1014 `update_patient_multistate(key_record, lang)` [callback]
- L1044 `update_patient_independence(key_record, lang)` [callback]
- L1065 `update_patient_topography(key_record, modality, lang)` [callback]
- L1092 `update_patient_level_descent(key_record, lang)` [callback]
- L1129 `download_report(n_clicks, key_record, id_number, strata, lang)` [callback]

### simulator.py (922 lines)
Simulator tab — hypothetical patient prediction + What-if counterfactual.
- L79 `render_simulator(lang, ref_data)`
- L200 `_conversion_card(lang)` — Hypothetical AIS-grade conversion card driven by the simulator's admission input…
- L222 `_level_descent_card(lang)` — Hypothetical neurological-level descent card driven by the simulator's admission…
- L246 `_multistate_card(lang)` — Hypothetical AIS multi-state recovery card driven by the simulator's admission i…
- L270 `_independence_card(lang)` — Hypothetical functional-independence profile driven by the simulator's admission…
- L290 `_topo_worksheet(lang, seed)` — ISNCSCI-worksheet-style editable admission-grade grid: rows = cord levels (rostr…
- L328 `_topography_card(lang, ref_data)` — Hypothetical recovery-topography card: an editable per-segment admission workshe…
- L367 `_lm_obs_input(measure, lang)`
- L380 `_landmark_card(lang)` — Hypothetical dynamic-prediction card: pick a landmark, enter observed scores, se…
- L417 `_simulate_regression(bundle, X, lang)`
- L461 `_simulate_multiclass(bundle, X, lang)`
- L490 `_reliability_badge(a, lang)`
- L553 `simulate(num_vals, cat_vals, num_ids, cat_ids, outcome_key, lang, ref_data)` [callback]
- L641 `launch_whatif(n_clicks, key_record, id_number)` [callback]
- L690 `update_whatif_banner(ref_data, lang)` [callback]
- L714 `clear_whatif(_n)` [callback]
- L728 `fill_or_clear(_fill, _clear, num_ids, cat_ids)` [callback] — Fill every field with the cohort default, or clear all to blank (NaN).
- L752 `simulate_landmark(landmark, obs_vals, num_vals, cat_vals, outcome_key, lang, obs_ids, num_ids, cat_ids)` [callback]
- L783 `simulate_conversion(num_vals, cat_vals, lang, num_ids, cat_ids)` [callback]
- L808 `simulate_level_descent(num_vals, cat_vals, lang, num_ids, cat_ids)` [callback]
- L836 `simulate_multistate(num_vals, cat_vals, lang, num_ids, cat_ids)` [callback]
- L861 `simulate_independence(num_vals, cat_vals, lang, num_ids, cat_ids)` [callback]
- L882 `topo_seed_or_clear(_seed, _clear, seg_ids, ref_data)` [callback] — Seed the worksheet from the What-if reference patient's real admission exam, or …
- L904 `simulate_topography(num_vals, cat_vals, seg_vals, modality, lang, num_ids, cat_ids, seg_ids)` [callback]
