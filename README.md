...
> code, schema, and aggregated descriptive statistics, with *no* row-level patient
> data.
...
| `y_max_scim`       | best SCIM during the stay | n/a | auxiliary |
| `y_last_scim` / `y_last_timepoint` | last observed SCIM + which timepoint | n/a | auxiliary |
...
- **AIS ordinal-aware metrics:** alongside accuracy we report
  ...
  monitor *how far off* a misclassification lands, not merely how often one occurs.
...
baseline** is fit on the same cohort and split, and the landmark-minus-baseline gap
...
2. **Patient simulator** - every admission feature is exposed as a
   ...
   **Partial input (F25):** the form opens blank, so you supply only what
